...
namespace MNIST_TensorFlowdotNet
{
    class Program
    {
        ...
        // Hyper-parameters
        static int epochs = 5; // accuracy > 98%
        static int batch_size = 100;
        static float learning_rate = 0.001f;
        static Datasets<MnistDataSet> mnist;

        // Network configuration
        // 1st Convolutional Layer
        static int filter_size1 = 5;  // Convolution filters are 5 x 5 pixels.
        static int num_filters1 = 16; //  There are 16 of these filters.
        static int stride1 = 1;  // The stride of the sliding window

        // 2nd Convolutional Layer
        static int filter_size2 = 5; // Convolution filters are 5 x 5 pixels.
        static int num_filters2 = 32;// There are 32 of these filters.
        static int stride2 = 1;  // The stride of the sliding window

        // Fully-connected layer.
        static int h1 = 128; // Number of neurons in fully-connected layer.

        static Tensor x, y;
        static Tensor loss, accuracy, cls_prediction;
        static Operation optimizer;

        static int display_freq = 100;
        static float accuracy_test = 0f;
        static float loss_test = 1f;

        static NDArray x_train, y_train;
        static NDArray x_valid, y_valid;
        static NDArray x_test, y_test;
        ...
        public static void Train(Session sess)
        {
        ...
        public static void Test(Session sess)
        {
        ...
        private static Tensor conv_layer(Tensor x, int filter_size, int num_filters, int stride, string name)
        {
        ...
        /// <param name="name">layer name</param>
        /// <returns>The output array</returns>
        private static Tensor max_pool(Tensor x, int ksize, int stride, string name)
        {
        ...
        private static Tensor flatten_layer(Tensor layer)
        {
        ...
        /// <returns></returns>
        private static RefVariable weight_variable(string name, int[] shape)
        {
        ...
        /// <param name="shape"></param>
        /// <returns></returns>
        private static RefVariable bias_variable(string name, int[] shape)
        {
        ...
        /// <returns>The output array</returns>
        private static Tensor fc_layer(Tensor x, int num_units, string name, bool use_relu = true)
        {
            return tf_with(tf.variable_scope(name), delegate
            {
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private static (NDArray, NDArray) Reformat(NDArray x, NDArray y)
        {
        ...
        public static Graph ImportGraph() => throw new NotImplementedException();

        public static void Predict(Session sess) => throw new NotImplementedException();
